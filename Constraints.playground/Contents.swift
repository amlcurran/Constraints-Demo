...
descriptionLabel.text = "A long description"
let hostingView = UIView()
hostingView.addSubview(titleLabel)
hostingView.addSubview(descriptionLabel)
titleLabel.constrainToSuperview([.leading, .trailing, .top], insetBy: 16)
descriptionLabel.constrainToSuperview([.leading, .trailing], insetBy: 16)
descriptionLabel.constrain(.top, to: titleLabel, .bottom, withOffset: 16)

hostingView.frame = CGRect(x: 0, y: 0, width: 300, height: 300)
hostingView.backgroundColor = .white
XCPlaygroundPage.currentPage.liveView = hostingView
